...
const Cart = () => {
    const { cart, setCart, addProduct, removeProduct, tonClient, isVipStatus } = useApp();
    ...
    const totalCost =
        Math.round(
            Object.values(cart).reduce((sum, item) => {
                const vipPrice = item.price * item.discount;
                return sum + (isVipStatus ? vipPrice : item.price) * item.quantity;
            }, 0) * 100,
        ) / 100;
    ...
    useMainButton(
        walletAddress
            ? isEmptyCart
                ? { text: 'Go to shop', onClick: () => navigate('/') }
