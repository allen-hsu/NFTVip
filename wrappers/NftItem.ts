...
export class NftItem implements Contract {
    ...
    async getNftData(provider: ContractProvider): Promise<{
        ...
        const stack = await nft_data.stack;
        let init = await stack.readNumber();
        let index = stack.readBigNumber();
        let collectionAddress = await stack.readAddress();
        let ownerAddress = await stack.readAddress();
        let content = await stack.readCell();
        let pointValue = await stack.readNumber();
        return {
            init: init,
            index: index,
